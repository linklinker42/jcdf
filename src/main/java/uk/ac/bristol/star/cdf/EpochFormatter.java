...
public class EpochFormatter {
  ...
  private static final Logger LOGGER = Logger.getLogger(EpochFormatter.class.getName());
  ...
  private static final long LAST_KNOWN_LEAP_UNIX_MILLIS = getLastKnownLeapUnixMillis(TT_SCALERS);
  /**
   ...
   */
  public static final boolean FAIL_ON_LEAP_ERROR = true;
  /**
   * 0 A.D. in Unix milliseconds as used by EPOCH/EPOCH16 data types.
   */
  public static final long AD0_UNIX_MILLIS = getAd0UnixMillis();

  private final DateFormat epochMilliFormat_ = createDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS");
  private final DateFormat epochSecFormat_ = createDateFormat("yyyy-MM-dd'T'HH:mm:ss");
   ...
   */
  public EpochFormatter(final int leapSecondLastUpdated) {
    final long lastDataLeapUnixMillis = getLastDataLeapUnixMillis(leapSecondLastUpdated);
    ...
    if (lastDataLeapUnixMillis > LAST_KNOWN_LEAP_UNIX_MILLIS
      ...
      final DateFormat fmt = createDateFormat("yyyy-MM-dd");
    ...
    if (lastDataLeapUnixMillis > 0) {
      final long lastDataLeapTt2kMillis
   ...
   * @return date string
   */
  public String formatEpoch(final double epoch) {
    final long unixMillis = (long) (epoch + AD0_UNIX_MILLIS);
    final Date date = new Date(unixMillis);
   ...
   * @return date string
   */
  public String formatEpoch16(final double epoch1, final double epoch2) {
    final long unixMillis = (long) (epoch1 * 1000) + AD0_UNIX_MILLIS;
    final Date date = new Date(unixMillis);
    final long plusPicos = (long) epoch2;
    if (plusPicos < 0 || plusPicos >= 1e12) {
      return "??";
    }
    final String result = new StringBuffer(32)
   ...
   * @return date string
   */
  public String formatTimeTt2000(final long timeTt2k) {
    ...
    final int scalerIndex = getScalerIndex(tt2kMillis);
    ...
    final TtScaler scaler = TT_SCALERS[scalerIndex];

    // Use it to convert to Unix time, which is UTC.
    final long unixMillis = (long) scaler.tt2kToUnixMillis(tt2kMillis);
    final int leapMillis = scaler.millisIntoLeapSecond(tt2kMillis);
    ...
    // in a minute.
      ...
      final Date date = new Date(unixMillis - 1000);
      txt = epochMilliFormat_.format(date)
        .replaceFirst(":59\\.", ":60.");
    }
   ...
   */
  private int getScalerIndex(final long tt2kMillis) {

    // Use the most recently used value as the best guess.
    // There's a good chance it's the right one.
    final int index = TtScaler.getScalerIndex(tt2kMillis, TT_SCALERS, iLastTtScaler_);
  ...
  private static long getAd0UnixMillis() {
    final GregorianCalendar cal = new GregorianCalendar(UTC, Locale.UK);
    cal.setLenient(true);
    cal.clear();
    cal.set(0, 0, 1, 0, 0, 0);
    final long ad0 = cal.getTimeInMillis();
    ...
    final long fudge = 1000 * 60 * 60 * 24 * 2;  // 2 days
    return ad0 + fudge;
  }
   ...
   */
  private static String prePadWithZeros(final long value, final int leng) {
    final String txt = Long.toString(value);
    final int nz = leng - txt.length();
    if (nz == 0) {
      return txt;
    } else if (nz < 0) {
      throw new IllegalArgumentException();
    } else {
      final StringBuffer sbuf = new StringBuffer(leng);
      for (int i = 0; i < nz; i++) {
        sbuf.append('0');
      }
      sbuf.append(txt);
   ...
   */
  private static long getLastKnownLeapUnixMillis(final TtScaler[] scalers) {
    final TtScaler lastScaler = scalers[scalers.length - 1];
    return (long) lastScaler.tt2kToUnixMillis(lastScaler.getFromTt2kMillis());
  }
   ...
   */
  private static long getLastDataLeapUnixMillis(final int leapSecondLastUpdated) {
    if (leapSecondLastUpdated == 0) {
      return Long.MIN_VALUE;
    } else if (leapSecondLastUpdated == -1) {
      return Long.MIN_VALUE;
    } else {
      final DateFormat fmt = createDateFormat("yyyyMMdd");
      try {
        return fmt.parse(Integer.toString(leapSecondLastUpdated))
          .getTime();
      } catch (ParseException e) {
        LOGGER.warning("leapSecondLastUpdated="
