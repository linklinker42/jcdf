...
public abstract class VariableDescriptorRecord extends Record {
   ...
   */
  private VariableDescriptorRecord(final RecordPlan plan, final String abbrev,
    final int recordType, final boolean hasDims, final int nameLeng)
    throws IOException {
    ...
    final boolean hasPad = hasBit(this.flags, 1);
    ...
    if (hasDims) {
      ...
      final long runningCount = plan.getReadCount(ptr);
      final long spareBytes = super.getRecordSize() - runningCount - padBytes_;
      assert spareBytes == (int) spareBytes;
      if (spareBytes % 4 != 0) {
        super.warnFormat("rVDR DimVarys field non-integer size??");
      }
      ndim = ((int) spareBytes) / 4;
    }
    final int[] iDimVarys = readIntArray(buf, ptr, ndim);
    this.dimVarys = new boolean[ndim];
    for (int i = 0; i < ndim; i++) {
      this.dimVarys[i] = iDimVarys[i] != 0;
    }
    final long padpos = ptr.getAndIncrement(padBytes_);
  ...
  public static class RVariant extends VariableDescriptorRecord {
     ...
     */
    public RVariant(final RecordPlan plan, final int nameLeng) throws IOException {
  ...
  public static class ZVariant extends VariableDescriptorRecord {
     ...
     * @throws java.io.IOException
     */
    public ZVariant(final RecordPlan plan, final int nameLeng) throws IOException {
