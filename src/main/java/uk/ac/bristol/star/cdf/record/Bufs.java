...
public class Bufs {
   ...
   * @return
   */
  public static Buf createBuf(final ByteBuffer byteBuffer,
   ...
   * @return
   */
  public static Buf createBuf(final ByteBuffer[] byteBuffers,
    final boolean isBit64, final boolean isBigendian) {
   ...
   */
  public static Buf createBuf(final File file,
    final boolean isBit64, final boolean isBigendian)
    throws IOException {
    final FileChannel channel = new FileInputStream(file).getChannel();
    final long leng = file.length();
    if (leng <= Integer.MAX_VALUE) {
      final int ileng = (int) leng;
      final ByteBuffer bbuf = channel.map(FileChannel.MapMode.READ_ONLY, 0, ileng);
   ...
   * @throws java.io.IOException
   */
  public static Buf uncompress(final Compression compression, final Buf inBuf,
    final long inOffset, final long outSize)
    throws IOException {
    LOGGER.config("Uncompressing CDF data to new " + outSize
      + "-byte buffer");
    try (
      final InputStream uin = compression.uncompressStream(
        new BufferedInputStream(
          inBuf.createInputStream(inOffset)))) {
      final Buf ubuf = inBuf.fillNewBuf(outSize, uin);
      return ubuf;
    }
  }
   ...
   * @return stream
   */
  public static InputStream createByteBufferInputStream(final ByteBuffer bbuf) {
   ...
   */
  static String readAsciiString(final ByteBuffer bbuf, final int ioff, final int nbyte) {
    final byte[] abuf = new byte[nbyte];
    synchronized (bbuf) {
      bbuf.position(ioff);
      bbuf.get(abuf, 0, nbyte);
    }
    final StringBuffer sbuf = new StringBuffer(nbyte);
   ...
   */
  static void readBytes(final ByteBuffer bbuf, final int ioff, final int count, final byte[] a) {
    if (count == 1) {
      a[0] = bbuf.get(ioff);
    } else {
      synchronized (bbuf) {
   ...
   * @param a     array into which values will be read, starting at element 0
   */
  static void readShorts(final ByteBuffer bbuf, final int ioff, final int count, final short[] a) {
    if (count == 1) {
   ...
   * @param a     array into which values will be read, starting at element 0
   */
  static void readInts(final ByteBuffer bbuf, final int ioff, final int count, final int[] a) {
    if (count == 1) {
   ...
   * @param a     array into which values will be read, starting at element 0
   */
  static void readLongs(final ByteBuffer bbuf, final int ioff, final int count, final long[] a) {
    if (count == 1) {
   ...
   * @param a     array into which values will be read, starting at element 0
   */
  static void readFloats(final ByteBuffer bbuf, final int ioff, final int count, final float[] a) {
    if (count == 1) {
   ...
   * @param a     array into which values will be read, starting at element 0
   */
  static void readDoubles(final ByteBuffer bbuf, final int ioff, final int count,
    final double[] a) {
    if (count == 1) {
  ...
  private static class ByteBufferInputStream extends InputStream {
     ...
     */
    ByteBufferInputStream(final ByteBuffer bbuf) {
      bbuf_ = bbuf;
    }
    ...
    public int read(final byte[] b, final int off, final int len) {
      ...
      final int remain = bbuf_.remaining();
      if (remain == 0) {
        return -1;
      } else {
        final int nr = Math.min(remain, len);
        bbuf_.get(b, off, nr);
        return nr;
      }
    }
    ...
    public long skip(final long n) {
      final int nsk = (int) Math.min(n, bbuf_.remaining());
