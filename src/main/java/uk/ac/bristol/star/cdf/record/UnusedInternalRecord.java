...
public class UnusedInternalRecord extends Record {
   ...
   */
  public UnusedInternalRecord(final RecordPlan plan) throws IOException {
    super(plan, "UIR", -1);
    final Buf buf = plan.getBuf();
    final Pointer ptr = plan.createContentPointer();
    final int planHeaderSize = (int) plan.getReadCount(ptr);
    ...
    final int pointerSize = buf.isBit64() ? 8 : 4;
    final int sociableUirSize = planHeaderSize + 2 * pointerSize;
