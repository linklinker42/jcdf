 ...
 *
 * <p>
 * To work out the buffer and offset from which to read a record value,
...
public class RecordMap {
   ...
   */
  private RecordMap(final Entry[] entries, final int recSize) {
    recSize_ = recSize;
   ...
   */
  public synchronized int getEntryIndex(final int irec) {
   ...
   * @see #getEntryIndex
   */
  public Buf getBuf(final int ient) {
    return bufs_[ient];
  }
   ...
   * @see #getEntryIndex
   */
  public long getOffset(final int ient, final int irec) {
   ...
   */
  public long getFinalOffsetInEntry(final int ient) {
   ...
   */
  private Block calculateBlock(final int irec) {
   ...
   */
  public static RecordMap createRecordMap(final VariableDescriptorRecord vdr,
    final RecordFactory recFact, final int recSize)
    throws IOException {
    final Compression compress = getCompression(vdr, recFact);
    final Buf buf = vdr.getBuf();
    ...
    for (long vxrOffset = vdr.vxrHead; vxrOffset != 0;) {
      final VariableIndexRecord vxr = recFact.createRecord(buf, vxrOffset,
   ...
   */
  private static Compression getCompression(final VariableDescriptorRecord vdr, final RecordFactory recFact)
    throws IOException {
    final boolean hasCompress = Record.hasBit(vdr.flags, 2);
    if (hasCompress && vdr.cprOrSprOffset != -1) {
      final CompressedParametersRecord cpr = recFact.createRecord(vdr.getBuf(), vdr.cprOrSprOffset,
   ...
   */
  private static void readEntries(final VariableIndexRecord vxr, final Buf buf,
    final RecordFactory recFact, final int recSize,
    final Compression compress, final List<Entry> list)
    throws IOException {
    ...
    final int nent = vxr.nUsedEntries;
    for (int ie = 0; ie < nent; ie++) {
      final int first = vxr.first[ie];
      final int last = vxr.last[ie];
      final Record rec = recFact.createRecord(buf, vxr.offset[ie]);

      // VVR: turn it directly into a new Entry and add to the list.
      if (rec instanceof VariableValuesRecord) {
        final VariableValuesRecord vvr = (VariableValuesRecord) rec;
        ...
        final CompressedVariableValuesRecord cvvr = (CompressedVariableValuesRecord) rec;
        final int uncompressedSize = (last - first + 1) * recSize;
        final Buf cBuf = Bufs.uncompress(compress, buf, cvvr.getDataOffset(),
          uncompressedSize);
      ...
      // Handle it with a recursive call to this routine.
        ...
        final VariableIndexRecord subVxr = (VariableIndexRecord) rec;
        readEntries(subVxr, buf, recFact, recSize, compress, list);
        for (long nextVxrOff = subVxr.vxrNext; nextVxrOff != 0;) {
          final VariableIndexRecord nextVxr = recFact.createRecord(buf, nextVxrOff,
            VariableIndexRecord.class);
  ...
  private static class Entry implements Comparable<Entry> {
     ...
     *               starts
     */
    Entry(final int first, final int last, final Buf buf, final long offset) {
      first_ = first;
      last_ = last;
      buf_ = buf;
      offset_ = offset;
   ...
   *
   * <p>
   * The <code>ient</code> member gives the index of the corresponding
   * Entry.
   ...
   *
   * <p>
   * Note that following the usage in VXR fields, the low and
  ...
  private static class Block {
     ...
     */
    Block(final int ient, final int low, final int high) {
      ient_ = ient;
      low_ = low;
      high_ = high;
    }
     ...
     */
    boolean contains(final int irec) {
   ...
   */
  private static int binarySearch(final int[] array, final int key) {
   ...
   */
  private static boolean isSorted(final int[] values) {
    final int nval = values.length;
