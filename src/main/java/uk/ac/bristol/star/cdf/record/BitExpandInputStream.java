...
abstract class BitExpandInputStream extends InputStream {
   ...
   */
  protected BitExpandInputStream(final InputStream base) {
    base_ = base;
    mask_ = 0x80;
  }
   ...
   */
  private static int read1(final InputStream in) throws IOException {
    final int b = in.read();
    if (b < 0) {
  ...
  public static class HuffmanInputStream extends BitExpandInputStream {
     ...
     * @param base compressed bit stream
     */
    public HuffmanInputStream(final InputStream base) throws IOException {
      super(base);
    ...
    private static Node[] inputCounts(final InputStream in) throws IOException {
      final Node[] nodes = new Node[514];
      for (int i = 0; i < 514; i++) {
        nodes[i] = new Node();
      }
      int ifirst = read1(in);
    ...
    private static int buildTree(final Node[] nodes) {
      int min1;
      int min2;
  ...
  public static class AdaptiveHuffmanInputStream
     ...
     * @param base compressed bit stream
     */
    public AdaptiveHuffmanInputStream(final InputStream base) {
      super(base);
    ...
    private void addNewNode(final int c) {
      final int iLightestNode = nextFreeNode_ - 1;
      final int iNewNode = nextFreeNode_;
      final int iZeroWeightNode = nextFreeNode_ + 1;
      nextFreeNode_ += 2;
    ...
    private void updateModel(final int c) {
    ...
    private void swapNodes(final int i, final int j) {
      ...
      final Node temp = new Node(nodes_[i]);
    ...
    private void rebuildTree() {
      ...
      for (int i = nextFreeNode_ - 2; j >= ROOT_NODE; i -= 2, j--) {
        ...
        final int weight = nodes_[j].weight_;
    ...
    private static class Node {
      ...
      Node(final int child, final boolean childIsLeaf, final int weight, final int parent) {
        child_ = child;
