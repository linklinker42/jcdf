 ...
 *
 * <p>
 * Constructing an instance of this class reads enough of a file
...
public class CdfReader {
  ...
  private static final Logger LOGGER = Logger.getLogger(CdfReader.class.getName());
  ...
  public CdfReader(Buf buf) throws IOException {
    final Pointer ptr = new Pointer(0);

    // Read the CDF magic number bytes.
    final int magic1 = buf.readInt(ptr);
    final int magic2 = buf.readInt(ptr);
    final int offsetRec0 = (int) ptr.get();

    // Work out from that what variant (if any) of the CDF format
    // this file implements.
    final CdfVariant variant = decodeMagic(magic1, magic2);
    if (variant == null) {
    ...
    LOGGER.config("CDF magic number for " + variant.label_);
    LOGGER.config("Whole file compression: " + variant.compressed_);
    ...
    if (variant.compressed_) {
      ...
      final CompressedCdfRecord ccr = recordFactory_.createRecord(buf, offsetRec0,
        CompressedCdfRecord.class);
      final CompressedParametersRecord cpr = recordFactory_.createRecord(buf, ccr.cprOffset,
        CompressedParametersRecord.class);
      final Compression compress = Compression.getCompression(cpr.cType);
      ...
      assert prepad == 8;
      final Compression padCompress = new Compression("Padded " + compress.getName()) {
        @Override
        public InputStream uncompressStream(final InputStream in)
          throws IOException {
          final InputStream in1 = new ByteArrayInputStream(new byte[prepad]);
          final InputStream in2 = compress.uncompressStream(in);
    ...
    final boolean isSingleFile = Record.hasBit(cdr_.flags, 1);
    if (!isSingleFile) {
      throw new CdfFormatException("Multi-file CDFs not supported");
    }
    final NumericEncoding encoding = NumericEncoding.getEncoding(cdr_.encoding);
    final Boolean bigEndian = encoding.isBigendian();
    if (bigEndian == null) {
   ...
   * @throws java.io.IOException
   */
  public CdfReader(final File file) throws IOException {
   ...
   *         a CDF magic number
   */
  public static boolean isMagic(final byte[] intro) {
    if (intro.length < 8) {
      return false;
    }
   ...
   * @return int value
   */
  private static int readInt(final byte[] b, int ioff) {
   ...
   */
  private static CdfVariant decodeMagic(final int magic1, final int magic2) {
    final String label;
    final boolean bit64;
    final int nameLeng;
  ...
  private static class CdfVariant {
     ...
     */
    CdfVariant(final String label, final boolean bit64, final int nameLeng, final boolean compressed) {
      label_ = label;
      bit64_ = bit64;
      nameLeng_ = nameLeng;
