...
public abstract class Shaper {
   ...
   */
  protected Shaper(final int[] dimSizes, final boolean[] dimVarys) {
    dimSizes_ = dimSizes;
    dimVarys_ = dimVarys;
  }
   ...
   */
  public abstract Object shape(final Object rawValue, final boolean rowMajor);
   ...
   */
  public abstract int getArrayIndex(final int[] coords);
   ...
   * @return
   */
  public static Shaper createShaper(final DataType dataType, final int[] dimSizes, final boolean[] dimVarys,
    final boolean rowMajor) {
    int rawItemCount = 1;
    int shapedItemCount = 1;
    int nDimVary = 0;
    final int ndim = dimSizes.length;
    for (int idim = 0; idim < dimSizes.length; idim++) {
      final int dimSize = dimSizes[idim];
  ...
  private static class ScalarShaper extends Shaper {
     ...
     * @param dataType data type
     */
    ScalarShaper(final DataType dataType) {
    ...
    @Override
    public Object shape(final Object rawValue, final boolean rowMajor) {
      return dataType_.getScalar(rawValue, 0);
    }

    @Override
    public int getArrayIndex(final int[] coords) {
  ...
  private static class VectorShaper extends Shaper {
     ...
     */
    VectorShaper(final DataType dataType, final int itemCount) {
    ...
    @Override
    public Object shape(final Object rawValue, final boolean rowMajor) {
      final Object out = Array.newInstance(dataType_.getArrayElementClass(),
        itemCount_);
  ...
  private static class GeneralShaper extends Shaper {
     ...
     *                 true for row major, false for column major
     */
    GeneralShaper(final DataType dataType, final int[] dimSizes, final boolean[] dimVarys,
      final boolean rowMajor) {
      ...
      for (int idim = 0; idim < ndim_; idim++) {
        final int jdim = rowMajor ? ndim_ - idim - 1 : idim;
        final int dimSize = dimSizes[jdim];
        shapedItemCount *= dimSize;
        if (dimVarys[jdim]) {
          nDimVary++;
    ...
    public Object shape(final Object rawValue, final boolean rowMajor) {
      ...
      final int[] coords = new int[ndim_];
  ...
  private static class SimpleArrayShaper extends GeneralShaper {
     ...
     *                 true for row major, false for column major
     */
    public SimpleArrayShaper(final DataType dataType, final int[] dimSizes, final boolean rowMajor) {
    ...
    public Object shape(final Object rawValue, final boolean rowMajor) {
      if (rowMajor == rowMajor_) {
        final int count = Array.getLength(rawValue);
        final Object out = Array.newInstance(dataType_.getArrayElementClass(),
          count);
     ...
     */
    private static boolean[] trueArray(final int n) {
   ...
   * @return array class
   */
  private static Class<?> getArrayClass(final Class elementClass) {
