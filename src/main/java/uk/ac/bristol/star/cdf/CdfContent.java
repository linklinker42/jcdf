...
public class CdfContent {
   ...
   */
  public CdfContent(final CdfReader crdr) throws IOException {

    // Get basic information from reader.
    final Buf buf = crdr.getBuf();
    final RecordFactory recordFact = crdr.getRecordFactory();
    final CdfDescriptorRecord cdr = crdr.getCdr();

    // Get global descriptor record.
    final GlobalDescriptorRecord gdr = recordFact.createRecord(buf, cdr.gdrOffset,
      GlobalDescriptorRecord.class);

    // Store global format information.
    final boolean rowMajor = Record.hasBit(cdr.flags, 0);
    final int[] rDimSizes = gdr.rDimSizes;
    final int leapSecondLastUpdated = gdr.leapSecondLastUpdated;
    cdfInfo_ = new CdfInfo(rowMajor, rDimSizes, leapSecondLastUpdated);

    // Read the rVariable and zVariable records.
    final VariableDescriptorRecord[] rvdrs
      = walkVariableList(buf, recordFact, gdr.nrVars, gdr.rVdrHead);
    final VariableDescriptorRecord[] zvdrs
    ...
    // Variable objects.
    final VariableDescriptorRecord[] vdrs = arrayConcat(rvdrs, zvdrs);
    ...
    final AttributeDescriptorRecord[] adrs
    ...
    final List<GlobalAttribute> gAttList = new ArrayList<GlobalAttribute>();
    final List<VariableAttribute> vAttList = new ArrayList<VariableAttribute>();
    for (int ia = 0; ia < adrs.length; ia++) {
      final AttributeDescriptorRecord adr = adrs[ia];
      final AttributeEntry[] grEntries = walkEntryList(buf, recordFact,
        adr.nGrEntries, adr.maxGrEntry,
        adr.agrEdrHead, cdfInfo_);
      final AttributeEntry[] zEntries = walkEntryList(buf, recordFact,
        adr.nZEntries, adr.maxZEntry,
        adr.azEdrHead, cdfInfo_);
      final boolean isGlobal = Record.hasBit(adr.scope, 0);
      if (isGlobal) {
        // grEntries are gEntries
        final AttributeEntry[] gEntries = arrayConcat(grEntries, zEntries);
   ...
   * @return list of VDRs
   */
  private static VariableDescriptorRecord[] walkVariableList(final Buf buf, final RecordFactory recordFact,
    final int nvar, final long head) throws IOException {
    final VariableDescriptorRecord[] vdrs = new VariableDescriptorRecord[nvar];
    long off = head;
    for (int iv = 0; iv < nvar; iv++) {
      final VariableDescriptorRecord vdr = recordFact.createRecord(buf, off,
   ...
   * @return list of ADRs
   */
  private static AttributeDescriptorRecord[] walkAttributeList(final Buf buf, final RecordFactory recordFact,
    final int natt, final long head) throws IOException {
    final AttributeDescriptorRecord[] adrs = new AttributeDescriptorRecord[natt];
    long off = head;
    for (int ia = 0; ia < natt; ia++) {
      final AttributeDescriptorRecord adr = recordFact.createRecord(buf, off,
   ...
   * @return entry values
   */
  private static AttributeEntry[] walkEntryList(final Buf buf, final RecordFactory recordFact,
    final int nent, final int maxient, final long head, final CdfInfo info)
    throws IOException {
    final AttributeEntry[] entries = new AttributeEntry[maxient + 1];
    long off = head;
    for (int ie = 0; ie < nent; ie++) {
      final AttributeEntryDescriptorRecord aedr = recordFact.createRecord(buf, off,
   ...
   * @return entry value
   */
  private static AttributeEntry readEntry(final AttributeEntryDescriptorRecord aedr,
    final CdfInfo info) throws IOException {
    final DataType dataType = DataType.getDataType(aedr.dataType, info);
    final int nitem;
   ...
   */
  private static <T> T[] arrayConcat(final T[] a1, final T[] a2) {
    final int count = a1.length + a2.length;
    final List<T> list = new ArrayList<>(count);
    list.addAll(Arrays.asList(a1));
    list.addAll(Arrays.asList(a2));
    final Class eClazz = a1.getClass().getComponentType();
