 ...
 *
 * <p>
 * The compressed stream is just like the uncompressed one,
...
class RunLengthInputStream extends InputStream {
   ...
   */
  public RunLengthInputStream(final InputStream base, final byte rleVal) {
    base_ = base;
    rleVal_ = rleVal & 0xff;
  }
  ...
  public int read() throws IOException {
    if (vCount_ > 0) {
      ...
      final int b = base_.read();
      if (b == rleVal_) {
        final int c = base_.read();
        if (c >= 0) {
          vCount_ = c;
          return rleVal_;
        } else {
