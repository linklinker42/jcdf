...
public abstract class DataType {
  ...
  private final boolean hasMultipleElementsPerItem_;
   ...
   */
  private DataType(final String name, final int byteCount, final int groupSize,
    final Class<?> arrayElementClass, final Class<?> scalarClass,
    final Object dfltPadValueArray, final boolean hasMultipleElementsPerItem) {
    name_ = name;
   ...
   */
  public int getArrayIndex(final int itemIndex) {
   ...
   */
  public abstract void readValues(final Buf buf, final long offset, final int nelPerItem,
    final Object valueArray, final int count)
    throws IOException;
   ...
   *
   * <p>
   * The <code>arrayIndex</code> argument is the index into the
   ...
   */
  public abstract Object getScalar(final Object valueArray, final int arrayIndex);
   ...
   */
  public String formatScalarValue(final Object value) {
   ...
   * @return string representation
   */
  public String formatArrayValue(final Object array, final int arrayIndex) {
    final Object value = Array.get(array, arrayIndex);
    return value == null ? "" : value.toString();
  }
   ...
   *
   * <p>
   * Currently, this returns the same as <code>getDataType(int)</code>,
   ...
   *
   * @throws uk.ac.bristol.star.cdf.CdfFormatException
   */
  public static DataType getDataType(final int dataType, final CdfInfo cdfInfo)
   ...
   * @throws uk.ac.bristol.star.cdf.CdfFormatException
   */
  public static DataType getDataType(final int dataType)
    throws CdfFormatException {
    switch (dataType) {
      case 1:
        return INT1;
  ...
  private static final class Int1DataType extends DataType {
    ...
    Int1DataType(final String name) {
      super(name, 1, 1, byte.class, Byte.class);
    }

    @Override
    public void readValues(final Buf buf, final long offset, final int nelPerItem,
      final Object array, final int n) throws IOException {
      buf.readDataBytes(offset, n, (byte[]) array);
    }

    @Override
    public Object getScalar(final Object array, final int index) {
  ...
  private static final class Int2DataType extends DataType {
    ...
    Int2DataType(final String name) {
  ...
  private static final class Int4DataType extends DataType {
    ...
    Int4DataType(final String name) {
  ...
  private static class Int8DataType extends DataType {
    ...
    Int8DataType(final String name) {
  ...
  private static class UInt1DataType extends DataType {
    ...
    UInt1DataType(final String name) {
  ...
  private static class UInt2DataType extends DataType {
    ...
    UInt2DataType(final String name) {
    ...
    public void readValues(final Buf buf, final long offset, final int nelPerItem,
      ...
      final boolean bigend = buf.isBigendian();
      for (int i = 0; i < n; i++) {
        final int b0 = buf.readUnsignedByte(ptr);
        final int b1 = buf.readUnsignedByte(ptr);
  ...
  private static class UInt4DataType extends DataType {
    ...
    UInt4DataType(final String name) {
    ...
    public void readValues(final Buf buf, final long offset, final int nelPerItem,
      final Object array, final int n) throws IOException {
      final Pointer ptr = new Pointer(offset);
      final long[] larray = (long[]) array;
      final boolean bigend = buf.isBigendian();
      for (int i = 0; i < n; i++) {
        final long b0 = buf.readUnsignedByte(ptr);
        final long b1 = buf.readUnsignedByte(ptr);
        final long b2 = buf.readUnsignedByte(ptr);
        final long b3 = buf.readUnsignedByte(ptr);
        larray[i] = bigend
          ? b3 | (b2 << 8) | (b1 << 16) | (b0 << 24)
          : b0 | (b1 << 8) | (b2 << 16) | (b3 << 24);
      }
  ...
  private static class Real4DataType extends DataType {
    ...
    Real4DataType(final String name) {
  ...
  private static class Real8DataType extends DataType {
    ...
    Real8DataType(final String name) {
  ...
  private static class Tt2kDataType extends Int8DataType {
    ...
    private final long[] dfltPad_ = new long[]{Long.MIN_VALUE + 1};

    Tt2kDataType(final String name, final int leapSecondLastUpdated) {
      super(name);
    ...
    @Override
    public String formatArrayValue(final Object array, final int index) {
      synchronized (formatter_) {
        return formatter_
    ...
    public boolean equals(final Object o) {
      if (o instanceof Tt2kDataType) {
        final Tt2kDataType other = (Tt2kDataType) o;
        return this.leapSecondLastUpdated_ == other.leapSecondLastUpdated_;
      } else {
        return false;
      }
    }
  ...
  private static class CharDataType extends DataType {
    ...
    CharDataType(final String name) {
    ...
    public void readValues(final Buf buf, final long offset, final int nelPerItem,
      final Object array, final int n) throws IOException {
      final String[] sarray = (String[]) array;
      final byte[] cbuf = new byte[nelPerItem * n];
      buf.readDataBytes(offset, nelPerItem * n, cbuf);
      for (int i = 0; i < n; i++) {
        @SuppressWarnings("deprecation")
        final String s = new String(cbuf, i * nelPerItem, nelPerItem);
        sarray[i] = s;
      }
    }
  ...
  private static class EpochDataType extends Real8DataType {
    ...
    EpochDataType(final String name) {
      super(name);
    }
  ...
  private static class Epoch16DataType extends DataType {
    ...
    Epoch16DataType(final String name) {
    ...
    @Override
    public Object getScalar(final Object array, final int index) {
      final double[] darray = (double[]) array;
      return new double[]{darray[index], darray[index + 1]};
    }

    @Override
    public String formatScalarValue(final Object value) {
      final double[] v2 = (double[]) value;
      synchronized (formatter_) {
