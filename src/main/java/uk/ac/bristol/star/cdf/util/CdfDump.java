 ...
 *
 * <p>
 * The output can optionally be written in HTML format.
...
public class CdfDump {
   ...
   */
  public CdfDump(final CdfReader crdr, final PrintStream out, final boolean writeFields, final boolean html) {
    crdr_ = crdr;
    out_ = out;
  ...
  public void run() throws IOException {
    final Buf buf = crdr_.getBuf();
    final RecordFactory recFact = crdr_.getRecordFactory();
    long offset = 8;  // magic number
    final long leng = buf.getLength();
    long eof = leng;
    ...
    for (int ix = 0; offset < eof; ix++) {
      final Record rec = recFact.createRecord(buf, offset);
   ...
   */
  private void dumpRecord(final int index, final Record rec, final long offset) {
    ...
    if (writeFields_) {
      final Field[] fields = rec.getClass().getFields();
   ...
   *         record type
   */
  private boolean isCdfRecordField(final Field field) {
   ...
   */
  private boolean isOffsetField(final Field field) {
   ...
   */
  private String formatFieldValue(final String name, final Object value,
   ...
   */
  private String formatOffsetId(final long offset) {
    final String txt = "0x" + Long.toHexString(offset);
   ...
   * @return string for output
   */
  private String formatOffsetRef(final long offset) {
    final String txt = "0x" + Long.toHexString(offset);
   ...
   */
  static String spaces(final int count) {
    final StringBuffer sbuf = new StringBuffer(count);
